...
Theorem Z_congBool : forall (a b : Z) (f : Z -> Z), (Z.eqb a b) = true -> 
  (Z.eqb (f a) (f b) = true).
Proof.
  intros.
  smt.
...
Theorem Z_congProp : forall (a b : Z) (f : Z -> Z), a = b -> f a = f b.
Proof.
  smt.
Qed.
...
Theorem LEM_Z : forall (i : Z), i = 0%Z \/ ~(i = 0%Z).
Proof.
  smt.
Qed.

Theorem LEMBV : forall (b : bitvector 4),
  b = #b|0|0|0|0| \/ ~(b = #b|0|0|0|0|).
Proof.
  smt.
Qed.
(*************************************************************************)
...
(* CVC4 doesn't have a shl bit-blasting rule *)
Theorem BV_shl : forall (b : bitvector 4), 
  bv_shl b #b|0|0|0|0| = b.
Proof.
  smt.
Abort.


(**************** Passing recursive functions to SMT solver ****************)

Definition f' (x : Z) : Prop := True.
...
Definition f (x : Z) : Z := 1.
Theorem Z_congBool' : forall (a b : Z), (Z.eqb a b) = true -> 
  (Z.eqb (f a) (f b) = false).
Proof.
  (*smt.*)
Abort.
...
Theorem testPropFun : forall (a b : Z) (f : Z -> Prop), (Z.eqb a b) = true ->
  f a.
Proof.
  (*smt.*)
Abort.
...
(***************************** Example from Report *****************************)
Theorem F : forall (a b c : bool), (b && negb c) || (a && negb b) || negb a || c.
Proof.
  smt.
Qed.
...
Lemma const_fun_is_eq_val_0 :
  forall f : Z -> Z,
    (forall a b, f a =? f b = true) ->
    forall x, f x =? f 0 = true.
Proof.
  intros f Hf.
  verit Hf.
Qed.
